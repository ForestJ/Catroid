...
public class TransparentWhenBrickTest extends ActivityInstrumentationTestCase2<StageActivity> {
	...
	public void testTapOnFullTransparentAreaOfForegroundSprite() {
		...
		UiTestUtils.clickOnStageCoordinates(solo, 0, 0, screenWidth, screenHeight);
		solo.sleep(1000);
		assertEquals("Sprite cat is at false position", catXPosition, cat.look.getXInUserInterfaceDimensionUnit());
		assertEquals("Sprite cat is at false position", catYPosition, cat.look.getYInUserInterfaceDimensionUnit());
		assertEquals("Sprite fish has moved", 0, fish.look.getXInUserInterfaceDimensionUnit());
		assertEquals("Sprite fish has moved", 0, fish.look.getYInUserInterfaceDimensionUnit());
	}
